...
class TEMConfig:
    ...
    SOLIDITY_VIABLE       = 0.45
    EDGE_COVERAGE_VIABLE  = 0.25
    GRADIENT_CONTRAST_MIN = 8
    ECCENTRICITY_REVIEW   = 0.88

    # --- CNN ---
    CNN_MODEL_PATH        = os.getenv("CNN_MODEL_PATH", "ev_viability_best_model.h5")
    CNN_IMG_SIZE          = 128
    CNN_CONFIDENCE_MIN    = 0.55      # below this → needs_review from CNN
...
def majority_vote(votes: List[VoteResult]) -> Tuple[str, float, str]:
    """
    2-out-of-3 majority vote.
    If all three disagree → needs_review.
    ...
    winning_votes = [v for v in votes
